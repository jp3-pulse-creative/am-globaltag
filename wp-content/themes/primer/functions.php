	...
	register_nav_menus(
		...
		(array) apply_filters(
			...
			array(
				...
				'services'  => esc_html__('Services Menu', 'primer'),
...
function primer_scripts()
{
	...
	wp_enqueue_script('ajax-filter',  get_stylesheet_directory_uri() . '/library/js/filter.js', array('jquery'), '1.0', true);

	wp_enqueue_style('animate-css', get_template_directory_uri() . '/library/css/animate.min.css');

	wp_enqueue_style('aos-css', get_template_directory_uri() . '/library/css/aos.css');
	wp_enqueue_script('aos-js', get_template_directory_uri() . '/library/js/aos.js', array('jquery'), false, true);
...
// ALM - our team - order by meta field last_name

function my_alm_team($args, $id)
{
	$args['meta_key'] = 'last_name';
	return $args;
}
add_filter('alm_query_args_alm_team', 'my_alm_team', 10, 2);

// ALM - select categories - get rid of hyphens on children

add_filter('alm_filters_select_terms_indent', function () {
	return ''; // default = ' - '
});
...
{
	wp_enqueue_style('custom-admin-styles', get_template_directory_uri() . '/custom-admin.css');
	wp_enqueue_script('custom-admin-js', get_template_directory_uri() . '/custom-admin.js', array(), false, true,  '1.0');
}
add_action('admin_enqueue_scripts', 'admin_style');


// for flexible content: only include vimeo player script once if it's needed

function enqueue_vimeo_player_script()
{
...
add_action('wp_footer', 'conditionally_enqueue_vimeo_script');

function conditionally_enqueue_vimeo_script()
{
	global $vimeo_script_needed;
	if ($vimeo_script_needed) {
		enqueue_vimeo_player_script();
	}
}
...
{
	$string = preg_replace('/[^a-zA-Z0-9\s]/', '', $string); // Remove non-alphanumeric characters except spaces
