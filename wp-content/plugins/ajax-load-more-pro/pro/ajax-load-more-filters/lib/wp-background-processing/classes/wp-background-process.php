...
/**
 * Abstract ALM_Background_Process class.
 *
 * @abstract
 * @extends ALM_Async_Request
 */
abstract class ALM_Background_Process extends ALM_Async_Request {
	/**
		...
		if ( ! empty( $items ) ) {
			...
			$batches = array_map(
				static function ( $item ) use ( $column, $value_column, $allowed_classes ) {
					$batch            = new stdClass();
					$data             = static::maybe_unserialize( $item->{$value_column}, $allowed_classes );
					$batch->key       = $item->{$column};
					$batch->filter_id = static::get_filter_id( $data );
					$batch->data      = $data;
	...
	/**
	 * Get filter id from batch data.
	 *
	 * @param array $batch Batch data.
	 * @return string
	 */
	protected static function get_filter_id( $batch ) {
		if ( ! empty( $batch ) ) {
