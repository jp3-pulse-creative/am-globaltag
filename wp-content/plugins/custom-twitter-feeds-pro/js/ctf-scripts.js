  ...
  (function($){
          ...
          var a = jQuery,
              ...
              c = function() {
                ...
                return b.prototype.init = function() {
                  ...
                  if( this.album[this.currentImageIndex].iframe !== undefined && this.album[this.currentImageIndex].iframe.length ){
				  ...
				  $('#ctf_lightbox').removeClass('ctf-has-broadcast')

          //Broadcast Videos
				  if (this.album[this.currentImageIndex].broadcast === 'true'){
						jQuery('#ctf_lightbox').find(".ctf_broadcast_video").fadeIn("slow");
						let broadcastUrl = this.album[this.currentImageIndex].broadcastUrl;
						$('#ctf_lightbox').removeClass('ctf_video_lightbox').addClass('ctf-has-broadcast');

						const container = document.getElementById('ctf-broadcast-ctn');
            renderTwitterBroadCastVideo(container, broadcastUrl);
					}
    ...
    ctfFeed.prototype = {
      ...
      revealNewImages: function() {
        ...
        $self.find('.ctf-item.ctf-new').each(function (index) {
          ...
          $self.find('.ctf-tweet-broadcast a[data-disabled-lightbox="true"]').on('click',function(event) {
            event.preventDefault();
            let $broadcastLink = $(this),
                container = $broadcastLink.closest('.ctf-tweet-broadcast').get(0),
                broadcastUrl = $broadcastLink.attr('data-broadcast-url');
              if(container !== undefined) {
                renderTwitterBroadCastVideo(container, broadcastUrl, true)
              }
      ...
      getImageUrls: function ($item) {
        ...
        var feed = this,
            apiSourceSet = $mediaItem.attr('data-available-images') !== undefined ? JSON.parse($mediaItem.attr('data-available-images').replace(/\\\//g, '/')) : undefined,
            srcSet = apiSourceSet !== undefined && typeof apiSourceSet[0] !== 'undefined' ? apiSourceSet : apiSourceSet,
            id = $item.attr('id').replace('ctf_','');
      ...
      addFullFeatures: function() {
        ...
        $(feed.el).find('.ctf-tweet-media').each(function() {
          ...
          if ($this.find('.ctf-image').length > 1) {
            $this.find('.ctf-image').each(function(index) {
              if (index > 0) {
                if ($(this).find('img').length && $(this).find('img').attr('src').indexOf('img/placeholder.png') > -1 && typeof $(this).find('img').first().attr('data-full-image') !== 'undefined') {
                  var newUrl = $(this).find('img').first().attr('data-full-image');
                  $(this).find('img').first().attr('src',newUrl).css('opacity',1);
                  $(this).find('a').first().css('background-image', 'url("' + newUrl + '")').attr('data-bg',newUrl);
                }
              }
    ...
    ctfLightboxBuilder.prototype = {
      getData: function(a){
        ...
        if (typeof a.attr("data-id") !== 'undefined') {
          var video = a.attr("data-video"),
              ...
              broadcastUrl = a.attr('data-broadcast-url'),
              link = a.attr("href");
          ...
          return {
            ...
            broadcast: a.attr('data-broadcast'),
      ...
      afterSlideChange: function () {
        ...
        $lightbox.find('.ctf_broadcast_video').hide();
    ...
    function renderTwitterBroadCastVideo(container, broadcastUrl, resize = false) {
			container.innerHTML = '';
      ...
      const link = document.createElement('a');
			link.href = broadcastUrl;
			blockquote.appendChild(link);
			container.appendChild(blockquote);

			if (window.twttr && window.twttr.widgets && typeof twttr.widgets.load === "function") {
        if (!resize) {
          twttr.widgets.load(container);
            ...
            const onRender = function(event) {
              if (container.contains(event.target)) {
                ...
                twttr.events.unbind('rendered', onRender);
              }
            };
