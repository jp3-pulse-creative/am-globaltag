...
class CTF_Upgrader_Pro
{
	...
	const INSTALL_INSTRUCTIONS = 'https://smashballoon.com/doc/setting-up-the-custom-twitter-feeds-pro-wordpress-plugin/?twitter';
	...
	{
		add_action('wp_ajax_nopriv_ctf_run_one_click_upgrade', array('TwitterFeed\Admin\CTF_Upgrader_Pro', 'install_upgrade'));
		add_action('wp_ajax_ctf_maybe_upgrade_redirect', array('TwitterFeed\Admin\CTF_Upgrader_Pro', 'maybe_upgrade_redirect'));
		add_action('sb_facebook_twicedaily', array('TwitterFeed\Admin\CTF_Upgrader_Pro', 'check_license_upgraded_cron' ));
	}
	...
	public static function maybe_upgrade_redirect()
	{
		...
		if (CTF_Upgrader_Pro::is_dev_url(home_url())) {
		...
		if (!is_wp_error($response)) {
			...
			if (empty($check_key_response['license_data'])) {
				wp_send_json_error(
					array(
						'message' => esc_html(CTF_Upgrader_Pro::get_error_message($check_key_response)),
					)
				);
			}
			...
			if (!empty($check_key_response['license_data']['error'])) {
				wp_send_json_error(
					array(
						'message' => CTF_Upgrader_Pro::get_error_message($check_key_response),
					)
				);
			}
			...
			$version      = '1.0';
			$version_info = CTF_Upgrader_Pro::get_version_info($license_data);
			$file         = '';
			...
			wp_send_json_success(
				array(
					'url' => $url,
					'same_version' => version_compare(CTF_VERSION, $check_key_response['current_version'], '='),
	...
	public static function install_upgrade()
	{
		...
		$installer = new \Ctf\Helpers\PluginSilentUpgrader(new \Ctf\Helpers\PluginSilentUpgraderSkin());
		...
		if (!empty($file)) {
			delete_option('ctf_islicence_upgraded');
			delete_option('ctf_upgraded_info');
	...
	public static function get_error_message($response)
	{
		...
		if (isset($response['license_data']['license']) && $response['license_data']['license'] === 'invalid') {
			$message = __('This license is NOT valid.', 'custom-twitter-feeds');
		}
	...
	public static function check_license_upgraded($current_license_data, $license)
	{
		...
		if (!is_wp_error($response)) {
			...
			if (isset($current_license_data['item_name'], $current_license_data['item_id'], $license_data['item_name'], $license_data['item_id']) && ($current_license_data['item_name'] !== $license_data['item_name'])) {
				update_option('ctf_islicence_upgraded', true);
				update_option('ctf_upgraded_info', $license_data);
			}
		}
	}
	...
	public static function check_license_upgraded_cron()
	{
		...
		if (isset($ctf_license_data['success'], $ctf_license_data['license']) && $ctf_license_data['success'] === true && $ctf_license_data['license'] === 'valid') {
			CTF_Upgrader_Pro::check_license_upgraded($ctf_license_data, $ctf_license_data['key']);
		}
	}
