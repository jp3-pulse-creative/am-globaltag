...
class CTF_Display_Elements_Pro {
	...
	public static function get_twitter_card_media_html( $data ) {
		...
		if ( ! empty( $image ) ) {
			$decoding = !empty($data['type']) && $data['type'] === 'broadcast_video'
				? 'decoding="async"'
				: '';
			return '<img src="' . esc_attr( $image ) . '" alt="' . esc_attr( $alt ) . '"' . $available_images . ' ' . $decoding . '>';
		}
	...
	public static function get_twitter_card_parts( $url, $twitter_card ) {
		...
		if ( ! empty( $title )
			...
			$parts = array(
				...
				'link_html'   => $link_html,
				'local'       => $local,
				'embed_url'   =>  $type === 'broadcast_video' && !empty($twitter_card['twitter:embed_url'])
					? $twitter_card['twitter:embed_url']
					: null
			);
		}
	...
	public static function get_twitter_card_html( $parts, $twittercards_attr, $feed_options = [] ) {
        if ( empty( $parts['url'] ) ) {
            return '';
        }
		...
		} elseif ($type === 'broadcast_video'){

			wp_enqueue_script('ctf-twtt-widgets');
			$disablelightbox = $feed_options['disablelightbox'];
			$embed_url = $parts['embed_url'];

			if (!empty($image_url)) {
				$lightbox_attr = !$disablelightbox
					? 'data-ctf-lightbox="1"'
					: "data-disabled-lightbox='true'";

				$card_html .= '<div class="ctf-tweet-broadcast">';
					$card_html .= '<a
						...
						data-date=""
						data-video=""
						data-broadcast-url="' . esc_url($embed_url) . '"
						data-broadcast="true"
						class="ctf-video"
					>';
						$card_html .= ctf_get_fa_el('ctf_playbtn');
